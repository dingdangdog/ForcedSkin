  ...
  <div class="max-w-4xl mx-auto px-4 py-10">
    ...
    <div class="flex items-center justify-between mb-8">
      <div class="flex items-center gap-3">
        ...
        <div>
          <h1 class="text-xl font-bold text-foreground">{{ authUser?.name || "我的账号" }}</h1>
    ...
    <section class="mb-8 p-5 rounded-2xl border border-border bg-surface-muted/40">
      <h2 class="font-semibold text-foreground mb-3">当前主题设置</h2>
      <div class="grid grid-cols-2 gap-4">
        <div class="flex items-center gap-3">
          <div class="w-8 h-8 rounded-lg bg-amber-100 flex items-center justify-center text-base">☀️</div>
          <div>
            <p class="text-xs text-muted">亮色主题</p>
            <p class="text-sm font-medium text-foreground">{{ selectedLight || '默认薄荷绿' }}</p>
          </div>
        </div>
        <div class="flex items-center gap-3">
          <div class="w-8 h-8 rounded-lg bg-slate-700 flex items-center justify-center text-base">🌙</div>
          <div>
            <p class="text-xs text-muted">暗色主题</p>
            <p class="text-sm font-medium text-foreground">{{ selectedDark || '默认深林绿' }}</p>
          </div>
        </div>
      </div>
      <p class="text-xs text-muted mt-3">在扩展中登录后，以上设置将自动同步</p>
    </section>
